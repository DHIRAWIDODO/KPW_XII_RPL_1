            ...
            <form action="{{ route('film.update', $film) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <textarea name="ringkasan" rows="4"
                              class="form-control @error('ringkasan') is-invalid @enderror">{{ old('ringkasan', $film->ringkasan) }}</textarea>
                    @error('ringkasan')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>

                <div class="row">
                    <div class="col-md-6 mb-3">
                        <label class="form-label">Poster (kosongkan jika tidak diganti)</label>
                        <br>
                        <img src="{{ asset('storage/' . $film->poster) }}" style="height: 100px;" class="mb-2 rounded">
                        <input type="file" name="poster" accept="image/*"
                               class="form-control @error('poster') is-invalid @enderror">
                        @error('poster')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-6 mb-3">
                        <label class="form-label">Trailer (kosongkan jika tidak diganti)</label>
                        <br>
                        @if ($film->trailer)
                            <video src="{{ asset('storage/' . $film->trailer) }}" style="height: 100px;" class="mb-2 rounded" muted></video>
                        @endif
